...
class SubscriberApi implements IApi<ISubscriber> {
  ...
  delete(id: string): Promise<AxiosResponse<ISubscriber>> {
  ...
  getOne(id: string): Promise<AxiosResponse<ISubscriber>> {
    return httpClient.get(`${SubscriberApi.BASE_URL}/${id}`);
  }

  update(id: string, body: ISubscriber): Promise<AxiosResponse<ISubscriber>> {
